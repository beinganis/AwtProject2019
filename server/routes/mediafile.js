...
const fs = require('fs');
const Media = require('../models/mediafiles');
...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        cb(null,Date.now()+'.'+ file.originalname); // overwrites current file with same name!!!
    }
});
...
routerMedia.post('/upload', upload.single('file'), (req, res, next) => {
    ...
    Media.addNewFile(newFile, (err, result) => {
        if (err) {
            res.status(500).json({ success: false, msg: 'file Not added to DB. Error: ' + err });
        } else {
            res.status(200).json({ success: true, msg: 'file Added to DB! ' + result });
        }
    });
});
...
routerMedia.delete('/:id', (req, res, next) => {
    ...
    Media.remove({ _id: fileId }, (err, item) => {
        if (err) {
            ...
            fs.unlink('assets\/uploads\/' + filePath, () => {
