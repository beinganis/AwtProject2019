...
(function kvsortableModule(factory) {
    ...
    var dragEl, parentEl, ghostEl, cloneEl, rootEl, nextEl, lastDownEl, scrollEl, scrollParentEl, scrollCustomFn, lastEl, lastCSS, lastParentCSS, oldIndex, newIndex, activeGroup, putKvSortable, autoScroll = {}, tapEvt, touchEvt, moved, 
    /** @const */
    R_SPACE = /\s+/g, R_FLOAT = /left|right|inline/, expando = 'KvSortable' + (new Date).getTime(), win = window, document = win.document, parseInt = win.parseInt, setTimeout = win.setTimeout, $ = win.jQuery || win.Zepto, Polymer = win.Polymer, captureMode = false, passiveMode = false, supportDraggable = ('draggable' in document.createElement('div')), supportCssPointerEvents = (function (el) {
        // false when IE11
    ...
    })(), _silent = false, abs = Math.abs, min = Math.min, savedInputChecked = [], touchDragOverListeners = [], _autoScroll = _throttle(function (/**Event*/ evt, /**Object*/ options, /**HTMLElement*/ rootEl) {
    ...
    KvSortable.prototype = /** @lends KvSortable.prototype */ {
        ...
        _onDragOver: function (/**Event*/ evt) {
            var el = this.el, target, dragRect, targetRect, revert, options = this.options, group = options.group, activeKvSortable = KvSortable.active, isOwner = (activeGroup === group), isMovingBetweenKvSortable = false, canSort = options.sort;
            ...
            if (activeKvSortable && !options.disabled &&
                ...
                if ((el.children.length === 0) || (el.children[0] === ghostEl) ||
                    ...
                    targetRect = target.getBoundingClientRect();
                    var width = targetRect.right - targetRect.left, height = targetRect.bottom - targetRect.top, floating = R_FLOAT.test(lastCSS.cssFloat + lastCSS.display)
                        || (lastParentCSS.display == 'flex' && lastParentCSS['flex-direction'].indexOf('row') === 0), isWide = (target.offsetWidth > dragEl.offsetWidth), isLong = (target.offsetHeight > dragEl.offsetHeight), halfway = (floating ? (evt.clientX - targetRect.left) / width : (evt.clientY - targetRect.top) / height) > 0.5, nextSibling = target.nextElementSibling, after = false;
                    if (floating) {
    ...
    function _dispatchEvent(kvsortable, rootEl, name, targetEl, toEl, fromEl, startIndex, newIndex) {
        ...
        var evt = document.createEvent('Event'), options = kvsortable.options, onName = 'on' + name.charAt(0).toUpperCase() + name.substr(1);
    ...
    function _onMove(fromEl, toEl, dragEl, dragRect, targetEl, targetRect, originalEvt, willInsertAfter) {
        var evt, kvsortable = fromEl[expando], onMoveFn = kvsortable.options.onMove, retVal;
